...
use BlockParty\Events\EmptyArenaChooser;
...
class BlockParty extends PluginBase implements Listener
 {
  ...
  public function onEnable()
   {
    ...
    $this->getLogger()
      ->info(TextFormat::GREEN . " is Enable!");
   }
  ...
  public function initConfig()
   {
	...
	if (!is_file($this->getDataFolder() . "languages/VietNamese.yml"))
     {
      $this->saveResource("languages/VietNamese.yml");
     }
    if (!is_file($this->getDataFolder() . "languages/{$this
      ->cfg
      ->get('Language') }.yml"))
     {
      ...
      $this->getServer()
        ->getLogger()
        ->info("Selected language English");
     }
    else
     {
      $this->msg = new Config($this->getDataFolder() . "languages/{$this
        ->cfg
        ->get('Language') }.yml", Config::YAML);
      $this->getServer()
        ->getLogger()
        ->info("Selected language {$this
        ->cfg
        ->get('Language') }");
     }
   }
        ...
        if($this->checkFile($arena) === true)
         {
          ...
          $this->getLogger()
            ->info("$fname - " . TextFormat::GREEN . "loaded!");
         }
        else
         {
          $this->arenas[basename($file, ".yml") ]       = $arena->getAll();
          ...
          $this->getLogger()
            ->error("Arena \"$fname\" has not finished setting up the arena yet!");
         }
       }
     }
   }
  ...
  public function checkFile(Config $arena)
   {
    if (!(is_numeric($arena->getNested("signs.join_sign_x")) 
		&& is_numeric($arena->getNested("signs.join_sign_y")) 
	    && is_numeric($arena->getNested("signs.join_sign_z")) 
	    && is_numeric($arena->getNested("signs.return_sign_x")) 
	    && is_numeric($arena->getNested("signs.return_sign_y")) 
	    && is_numeric($arena->getNested("signs.return_sign_z")) 
	    && is_string($arena->getNested("arena.arena_world")) 
	    && is_numeric($arena->getNested("arena.join_position_x")) 
	    && is_numeric($arena->getNested("arena.join_position_y")) 
	    && is_numeric($arena->getNested("arena.join_position_z")) 
	    && is_numeric($arena->getNested("arena.lobby_position_x")) 
	    && is_numeric($arena->getNested("arena.lobby_position_y")) 
	    && is_numeric($arena->getNested("arena.lobby_position_z")) 
	    && is_numeric($arena->getNested("arena.first_corner_x")) 
	    && is_numeric($arena->getNested("arena.first_corner_z")) 
	    && is_numeric($arena->getNested("arena.second_corner_x")) 
	    && is_numeric($arena->getNested("arena.second_corner_z")) 
	    ...
	    && is_numeric($arena->getNested("arena.max_game_time")) 
	    && is_numeric($arena->getNested("arena.max_players")) 
	    && is_numeric($arena->getNested("arena.min_players")) 
	    && is_numeric($arena->getNested("arena.color_wait_time")) 
	    && is_numeric($arena->getNested("arena.floor_y"))))
     {
      return false;
     }
    if (!((strtolower($arena->get("type")) == "furious" || strtolower($arena->get("type")) == "stoned" || strtolower($arena->get("type")) == "classic") 
		&& (strtolower($arena->get("material")) == "wool" || strtolower($arena->get("material")) == "clay") 
	    && (strtolower($arena->getNested("signs.enable_status")) == "true" || strtolower($arena->getNested("signs.enable_status")) == "false") 
          ...
          if ($sender instanceof Player)
           {
            ...
            case "help":
              ...
              $msg = "§l§e-BLOCK PARTY-§r:§o";
              if ($sender->hasPermission('bp.command.lobby')) $msg .= $this->getMsg('lobby');
              if ($sender->hasPermission('bp.command.npc')) $msg .= $this->getMsg('npc');
      ...
      if (isset($this->setters[strtolower($p->getName()) ]['arena']) && isset($this->setters[strtolower($p->getName()) ]['type']))
       {
        $e->setCancelled(true);
        $b     = $e->getBlock();
        $arena = new ConfigArena\ConfigManager($this->setters[strtolower($p->getName()) ]['arena'], $this);
        if ($this->setters[strtolower($p->getName()) ]['type'] == "setjoinsign")
         {
          $arena->setJoinSign($b->x, $b->y, $b->z, $b
            ->level
            ->getName());
        ...
        if (count($args) >= 1 && count($args) <= 2)
         {
          if ($args[0] === 'help')
           {
            $help1     = $this->getMsg('help_joinsign') . $this->getMsg('help_returnsign') . $this->getMsg('help_startpos') . $this->getMsg('help_lobbypos') . $this->getMsg('help_corners') . $this->getMsg('help_spectatorspawn');
            $help2     = $this->getMsg('help_time') . $this->getMsg('help_colortime') . $this->getMsg('help_type') . $this->getMsg('help_material') . $this->getMsg('help_allowstatus') . $this->getMsg('help_statusline');
            $help3     = $this->getMsg('help_allowspectator') . $this->getMsg('help_signupdatetime') . $this->getMsg('help_maxtime') . $this->getMsg('help_maxplayers') . $this->getMsg('help_minplayers') . $this->getMsg('help_enable');
            $helparray = [$help1, $help2, $help3];
            if (isset($args[1]))
             {
              if (intval($args[1]) >= 1 && intval($args[1]) <= 3)
               {
                $help      = "§aYou need to complete the data of all the pages for the arena to be active\n--- §e§lBlockParty setup help $args[1]/3§r ---§f";
            ...
            $p->sendMessage("§aYou need to complete the data of all the pages for the arena to be active\n--- §e§lBlockParty setup help§l 1/3 §r---§f" . $help1);
            return;
           }
         }
        ...
        if ($ins instanceof Plugin && $ins->isEnabled())
         {
          ...
          $this->getServer()
            ->getLogger()
            ->info("Selected economy plugin: $plugin");
          return;
         }
       }
      $this->economy = null;
